...
//				register(test[i]);
	...
	public static void register(String fileName){
		...
		try{		
			File file = new File(peerInfo.local.completeSharedPath + 
					fileName);
			System.out.println();
			if(!file.exists()){
...
//				peerInfo.local.messageTable = new HashMap<Integer,MessageID>();
			...
			case 4:{
			    ...
			    //System.out.println("message read");
                            ...
                            broadcaster brd = new broadcaster(newMssgId,broadcastMessage,peerInfo.local.nick.peerName);
...
class DThread extends Thread{
	...
	public static void receiveFile(Socket socket, String fileName) throws IOException{
        ...
        String filePath = peerInfo.local.completeSharedPath + fileName;  
        try {  
            try {  
                dis = new DataInputStream(socket.getInputStream()); 
                System.out.println();
               
                File f = new File(peerInfo.local.completeSharedPath);  
